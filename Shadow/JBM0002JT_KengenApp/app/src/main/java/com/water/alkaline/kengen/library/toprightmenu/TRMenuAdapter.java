package com.water.alkaline.kengen.library.toprightmenu;
...
public class TRMenuAdapter extends RecyclerView.Adapter<TRMenuAdapter.TRMViewHolder> {
    ...
    private StateListDrawable addStateDrawable(Context context, int normalId, int pressedId){
        ...
        @SuppressLint("UseCompatLoadingForDrawables") Drawable normal = normalId == -1 ? null : context.getResources().getDrawable(normalId);
        @SuppressLint("UseCompatLoadingForDrawables") Drawable pressed = pressedId == -1 ? null : context.getResources().getDrawable(pressedId);
