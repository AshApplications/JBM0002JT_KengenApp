...
class DownloadAdapter(
    ...
    var arrayList: MutableList<DownloadEntity>,
    var listener: OnDownloadListener
    ...
    fun setAds(isAds: Boolean) {
        ...
        if (PARTICLE_AD_DISPLAY_COUNT > 0 && MyApp.getAdModel().adsOnOff.equals(
            ...
            val tempArr = mutableListOf<DownloadEntity>()

            if (arrayList.isNotEmpty()) {
                tempArr.add(DownloadEntity().apply {
                    id = Constant.defaultId.toInt()
                })
            }
    ...
    override fun getItemViewType(position: Int): Int {
        return if (arrayList[position].id.toString() == Constant.defaultId) Constant.AD_TYPE
    ...
    fun refreshAdapter(arrayList: MutableList<DownloadEntity>) {
