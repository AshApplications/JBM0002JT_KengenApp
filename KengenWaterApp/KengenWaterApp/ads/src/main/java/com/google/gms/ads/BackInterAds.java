...
public class BackInterAds {
    ...
    public void loadInterAds(Activity context) {
        ...
        if (mInterstitialAd == null && !interAd.equalsIgnoreCase("") && PowerPreference.getDefaultFile().getInt(AdUtils.SERVER_BACK_COUNT, 0) > 0 && PowerPreference.getDefaultFile().getBoolean(AdUtils.GoogleAdsOnOff)) {
    ...
    public void showQureka(Activity context) {
        if (PowerPreference.getDefaultFile().getBoolean(AdUtils.QurekaOnOff, false)) {
            ...
            intent.putExtra(AdUtils.BACK_ADS, true);
